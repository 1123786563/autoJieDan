...
async function configureProviders(config: AutomatonConfig): Promise<void> {
  ...
  config.zhipuApiKey = await askString("Zhipu AI API key  (智谱 GLM)", config.zhipuApiKey) || undefined;
  config.moonshotApiKey = await askString("Moonshot API key  (月之暗面 Kimi)", config.moonshotApiKey) || undefined;
