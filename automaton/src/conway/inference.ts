...
type InferenceBackend = "conway" | "openai" | "anthropic" | "ollama" | "zhipu" | "moonshot";

export function createInferenceClient(
  options: InferenceClientOptions,
): InferenceClient {
  const { apiUrl, apiKey, openaiApiKey, anthropicApiKey, ollamaBaseUrl, zhipuApiKey, moonshotApiKey, getModelProvider } = options;
  ...
  const chat = async (
    ...
    const openAiLikeApiUrl =
      ...
      backend === "moonshot" ? "https://api.moonshot.cn/v1" :
    ...
    const openAiLikeApiKey =
      ...
      backend === "moonshot" ? (moonshotApiKey as string) :
...
function resolveInferenceBackend(
  ...
  if (keys.getModelProvider) {
    ...
    if (provider === "moonshot" && keys.moonshotApiKey) return "moonshot";
  ...
  if (keys.moonshotApiKey && /^(moonshot|kimi)/i.test(model)) return "moonshot";
  return "conway";
...
async function chatViaOpenAiCompatible(params: {
  ...
  apiUrl: string;
  apiKey: string;
  backend: "conway" | "openai" | "ollama" | "zhipu" | "moonshot";
  ...
  const resp = await params.httpClient.request(`${params.apiUrl}/v1/chat/completions`, {
    ...
    headers: {
      ...
      Authorization:
        ...
        params.backend === "zhipu" || params.backend === "moonshot"
